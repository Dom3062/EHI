...
function EHIDamageFloatManager:damage_callback(c_dmg, damage_info)
    if damage_info.col_ray or damage_info.is_synced or damage_info.variant == "poison" or damage_info.variant == "graze" then
        ...
        if hitPos then
            ...
            local color = (tweak_data.chat_colors[pid] or Color.white):with_alpha(death and 1 or 0.5)--self:_color(pid, Color.white):with_alpha(death and 1 or 0.5)
            local texts = {}
            local n = 1
            if isCrit then
                texts[n] = { '', Color.red }
            ...
            table.insert(self.pops, EHIDamageFloat:new(self, { pos = hitPos, text = texts,
                crit = isCrit,
                t = isCrit and self._damage_decay * 1.2 or self._damage_decay
            }))
        end
    end
end
