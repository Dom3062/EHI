...
function ElementWaypoint:ehi_on_executed(instigator, ...)
    if not self._values.enabled then
        return
    elseif self._values.only_on_instigator and instigator ~= managers.player:player_unit() then
        ElementWaypoint.super.on_executed(self, instigator, ...)
        return
    elseif not self._values.only_in_civilian or managers.player:current_state() == "civilian" then
