...
local EHI = EHI
if EHI._hooks.ElementInteraction then
    return
else
    EHI._hooks.ElementInteraction = true
end
...
if level_id == "mex" then
    triggers = {
        [101413] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101422] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101423] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101425] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101431] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101433] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101434] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled },
        [101435] = { id = "mex_9", special_function = SFF.IncreaseProgressWhenElementIsEnabled }
    }
else
    return
end
...
for id, data in pairs(triggers) do
    if data.random_time then
        if data.class and data.class == TT.Warning then
...
local function Trigger(id, enabled)
    ...
    if triggers[id] then
        if triggers[id].special_function then
            ...
            elseif f == SFF.IncreaseProgressWhenElementIsEnabled then
                if enabled then
                    managers.hud:IncreaseProgress(triggers[id].id)
                end
            end
...
local _f_client_on_executed = ElementInteraction.client_on_executed
function ElementInteraction:client_on_executed()
    _f_client_on_executed(self)
    Trigger(self._id, self._values.enabled)
end

local _f_on_executed = ElementInteraction.on_executed
function ElementInteraction:on_executed(...)
    _f_on_executed(self, ...)
    Trigger(self._id, self._values.enabled)
end