...
local EHI = EHI
if EHI._hooks.ElementEnemyDummyTrigger then -- Don't hook multiple times, pls
    return
else
    EHI._hooks.ElementEnemyDummyTrigger = true
end
...
elseif level_id == "flat" then -- Panic Room
    ...
    triggers = {
        [100811] = { id = "ThugDeath", special_function = SF.IncreaseProgressMax },
        [100820] = { id = "ThugDeath", special_function = SF.IncreaseProgress }
    }
else
    return
end
    ...
    if triggers[id] then
        if triggers[id].special_function then
            ...
            elseif f == SF.IncreaseProgressMax then
                ...
                if managers.ehi:TrackerExists(t_id) then
                    managers.ehi:CallFunction(t_id, "IncreaseProgressMax")
                else
                    EHI._cache[t_id] = (EHI._cache[t_id] or 1) + 1
                end
...
local _f_client_on_executed = ElementEnemyDummyTrigger.client_on_executed
function ElementEnemyDummyTrigger:client_on_executed(instigator)
    _f_client_on_executed(self, instigator)
    Trigger(self._id)
end

local _f_on_executed = ElementEnemyDummyTrigger.on_executed
