...
local Force = -- To be removed when every heist moved from HUDManagerPD2
{
...
local ForceVanillaFunctions = true
...
elseif LootCounter[level_id] then
    check_type = check_types.BagsOnly
    tracker_id = "LootCounter"
elseif Force[level_id] then
    ForceVanillaFunctions = false
else
    return
end
