...
function IngameWaitingForPlayersState:at_exit(...)
    ...
    if EHI:IsAchievementLocked2("gage2_5") and CheckWeaponsType("lmg") then -- "The Eighth and Final Rule" achievement
        local function f()
            ...
            EHI:HookWithID(StatisticsManager, "killed", "EHI_gage2_5_killed", function (self, data)
                if data.variant ~= "melee" and data.weapon_unit:base():is_category("lmg") and not CopDamage.is_civilian(data.name) then
    ...
    if EHI:IsAchievementLocked2("gage3_6") and CheckWeaponsType("snp") then
        ...
        EHI:HookWithID(StatisticsManager, "killed", "EHI_gage3_3-6_killed", function (self, data)
            if data.variant ~= "melee" and data.weapon_unit:base():is_category("snp") and not CopDamage.is_civilian(data.name) and data.head_shot then
    ...
    if level == "sand" and EHI:IsOVKOrAbove(difficulty) and EHI:IsAchievementLocked2("sand_11") and CheckWeaponsType("snp") then -- "This Calls for a Round of Sputniks!" achievement
        ...
        EHI:HookWithID(StatisticsManager, "killed", "EHI_sand_11_killed", function (_, data)
            if data.variant ~= "melee" and data.weapon_unit:base():is_category("snp") then
