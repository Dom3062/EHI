...
function IngameWaitingForPlayersState:at_exit(...)
    ...
    if EHI:GetUnlockableAndOption("show_dailies") then
        ...
        if managers.challenge:can_progress_challenges() then
            ...
            if EHI:IsDailyJobAvailable("any_25_shield_kills") then
                ShowTrackerInLoud(function() -- Show progress when alarm went off
                    AddDailyJobTracker("any_25_shield_kills", "any_shield_kills")
                end)
                challenge_kills.any_shield_kills = "any_25_shield_kills"
            end
            if EHI:IsDailyJobAvailable("any_50_headshot_kills") then
                AddDailyJobTracker("any_50_headshot_kills", "any_headshot_kills")
                challenge_kills.any_headshot_kills = "any_50_headshot_kills"
            end
            if next(challenge_kills) then
                EHI:HookWithID(ChallengeManager, "award_progress", "EHI_DailyJob_AwardProgress", function(c, progress_id, amount)
                    local challenge = challenge_kills[progress_id]
                    if challenge then
        ...
        if next(daily) then
            EHI:HookWithID(CustomSafehouseManager, "award", "EHI_SFDaily_AwardProgress", function(csm, id)
                if daily[id] then
