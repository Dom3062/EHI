...
EHIWaypointManagerVR = EHIWaypointManager
EHIWaypointManagerVR.old_SetPlayerHUD = EHIWaypointManager.SetPlayerHUD
EHIWaypointManagerVR.old_SetWaypointColor = EHIWaypointManager.SetWaypointColor

function EHIWaypointManagerVR:SetPlayerHUD(hud, saferect, gui)
    self._gui = gui
    self:old_SetPlayerHUD(hud, saferect, gui)
end

function EHIWaypointManagerVR:AddWaypoint(id, params)
    ...
    local ws = self._gui:create_world_workspace(128, 64, (params.position or params.unit:position()) + Vector3(40, 0, 20), Vector3(-80, 0, 0), Vector3(0, 0, -40))
    ...
    local timer = params.time and ws:panel():text({
        font_size = 32,
        h = 32 * self._scale,
        vertical = "center",
        w = 32 * self._scale,
...
function EHIWaypointManagerVR:RemoveWaypoint(id)
    ...
    self._gui:destroy_workspace(self._waypoints[id].ws)

    self._waypoints[id] = nil
end

function EHIWaypointManager:SetWaypointColor(id, color, override_base_color)
    self:old_SetWaypointColor(id, color, override_base_color)
    if not (id and self._waypoints[id]) then
        return
    end
    self._waypoints[id].bitmap_world:set_color(color)
end

function EHIWaypointManagerVR:AnimateWarning(waypoint)
    local icon = waypoint.bitmap
    local arrow = waypoint.arrow
    local bitmap_world = waypoint.bitmap_world
...
function EHIWaypointManagerVR:update(t, dt, dt_actual)
	...
	for id, data in pairs(self._waypoints) do
		...
		elseif data.state == "present" then
			...
			if dot < 0 or panel:outside(mvector3_x(wp_pos), mvector3_y(wp_pos)) then
				...
				mvector3_set_static(target_pos, panel_center_x + mvector3.x(direction) * distance, panel_center_y + mvector3.y(direction) * distance, 0)
				...
				data.arrow:set_center(mvector3_x(data.current_position) + direction.x * offset, mvector3_y(data.current_position) + direction.y * offset)
